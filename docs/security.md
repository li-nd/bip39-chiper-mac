...
4. Encode as a fixed **8-character** string in a custom Base32-like alphabet
...
Encoding treats the 5-byte prefix as a big-endian integer, reduces modulo 32⁸, and maps digits to alphabet indices. Leading zeros pad with the first alphabet character.
...
3. For each input code (any order):
   ...
   - Validate: exactly 8 valid alphabet characters
...
Theoretically two different `(position, word)` pairs could produce the same 8-character token (birthday bound on truncated HMAC). The app detects **ambiguous** tokens (multiple matches for the same slot) and **slot conflicts** and reports an error instead of silently picking one.
...
| Information leaked | Token space is small (8 chars, 30^8 possibilities per slot) | Ciphertext hides structure |
