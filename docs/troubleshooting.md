...
Codes must be exactly **8 characters** from the allowed alphabet after normalization:
...
Rare HMAC truncations can map two different `(position, word)` pairs to the same 8-character code. The app stops with an explicit error instead of guessing.
