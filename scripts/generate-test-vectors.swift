...
let referenceTokens = ["3JKDEPFPN", "2MKGTQMA4", "3MKJ78HRT", "3947VKVAG", "2GBM9FKQW"]
...
for (si, sep) in separators.enumerated() {
    let spaced = chunkJoin("3JKDEPFPN", sep: sep)
...
for length in 1...8 {
    let input = String("3JKDEPFPN".prefix(length))
...
for (pi, prefix) in ["", " ", "  ", "\t", ">>> ", "(", ") "].enumerated() {
    for (si, suffix) in ["", " ", "!!", "..."].enumerated() {
        ...
        let input = prefix + "3jkdepfpn" + suffix
        let normalized = normalizeToken(input)
        normalizeVectors.append(NormalizeVector(
            id: "norm-wrap-\(pi)-\(si)",
            input: input,
...
while normalizeVectors.count < 70 {
    ...
    let input = "3jkdepfpn" + String(repeating: " ", count: n % 4)
    let normalized = normalizeToken(input)
    normalizeVectors.append(NormalizeVector(
        id: "norm-pad-\(n)",
        input: input,
